...
use crate::tui::app::{
    ActivePane, App, DialogType, EditingField, InputMode, RequestTab, SidebarTab,
};
                ...
                if expanded {
                    for req in &col.requests {
                        ...
                        let mut spans = vec![
                            ...
                            Span::raw(format!(" {}", crate::utils::truncate(&req.name, 18))),
                        ];
                        // 在请求名后显示彩色标签
                        for tag in &req.tags {
                            let tcolor = crate::models::tag_color(tag);
...
fn draw_status_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    let (msg, style) = if app.loading {
        ...
        let tag_info = match &app.tag_filter {
            Some(t) => format!("Tag:{}", t),
            None => "Tag:-".to_string(),
        };
        let info = format!(
