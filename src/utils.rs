...
mod tests {
    ...
    fn test_builtin_timestamp() {
        let result = resolve_builtin_variables("ts={{$timestamp}}");
        assert!(result.starts_with("ts="));
        let ts_str = &result[3..];
        let ts: u128 = ts_str.parse().unwrap();
        assert!(ts > 1_700_000_000_000); // 应该是合理的毫秒时间戳
    }

    #[test]
    fn test_builtin_uuid() {
        let result = resolve_builtin_variables("id={{$uuid}}");
        assert!(result.starts_with("id="));
        let uuid_str = &result[3..];
        assert_eq!(uuid_str.len(), 36); // UUID 格式: 8-4-4-4-12
        assert!(uuid_str.contains('-'));
    }

    #[test]
    fn test_builtin_random_int() {
        let result = resolve_builtin_variables("n={{$randomInt}}");
        assert!(result.starts_with("n="));
        let n: u64 = result[2..].parse().unwrap();
        assert!(n < 10000);
    }

    #[test]
    fn test_builtin_random_str() {
        let result = resolve_builtin_variables("s={{$randomStr}}");
        assert!(result.starts_with("s="));
        assert_eq!(result.len(), 10); // "s=" + 8 chars
    }

    #[test]
    fn test_builtin_date() {
        let result = resolve_builtin_variables("d={{$date}}");
        assert!(result.starts_with("d="));
        let date_str = &result[2..];
        assert_eq!(date_str.len(), 10); // YYYY-MM-DD
        assert!(date_str.contains('-'));
    }

    #[test]
    fn test_builtin_no_match() {
        let result = resolve_builtin_variables("no vars here");
